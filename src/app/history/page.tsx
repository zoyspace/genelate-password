...
export default function HistoryPage() {
	...
	const { passwordHistory, loadingHistory, removeFromHistory, toggleFavorite, syncWithSupabase } = usePassword();
			...
			<div className="container mx-auto px-4 py-8">
				<div className="flex justify-between items-center mb-8">
					...
					<div className="flex items-center gap-4">
						<AuthButton />
						<Link href="/" className="px-4 py-2 bg-gray-200 text-gray-800 rounded-md hover:bg-gray-300 transition">
							ホームに戻る
						</Link>
					</div>
				</div>
				...
				{isLoggedIn ? (
					<div className="bg-green-100 border-l-4 border-green-500 text-green-700 p-4 mb-6">
						...
						<p>お気に入りのパスワードはクラウドに保存され、複数のデバイスで共有できます。</p>
					</div>
				) : (
					<div className="bg-blue-100 border-l-4 border-blue-500 text-blue-700 p-4 mb-6">
						<p className="font-bold">ヒント</p>
						<p>ログインすると、お気に入りのパスワードをクラウドに保存し、複数のデバイスで共有できます。</p>
					</div>
				)}
				...
				{!loadingHistory && passwordHistory.length === 0 ? (
					<div className="text-center py-12 bg-gray-50 rounded-lg">
						...
						<p className="mt-2 text-gray-500">パスワードを生成するとここに表示されます</p>
					</div>
				) : (
								...
								<div className="flex justify-between items-center mb-2">
									...
									<div className="flex gap-2">
										<button
											...
											title={entry.isFavorite ? "お気に入りから削除" : "お気に入りに追加"}
										>
											{entry.isFavorite ? (
												<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="currentColor" stroke="currentColor" strokeWidth="2" className="text-yellow-500">
